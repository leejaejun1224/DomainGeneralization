...
from models.losses.segmentloss import *
from models.tools.prior_setting import *
from models.tools.filter import *
### if student => model = 's'
### if teacher => model = 't'

# 선택: TF32 허용(속도 향상, 정확도 영향 미미)
...
class StereoDepthUDA(StereoDepthUDAInference):
    def __init__(self, cfg, student_optimizer=0.0, teacher_optimizer=0.0):
        ...
        self.amp_enabled = True  # cfg에서 가져오면 더 좋음
        # BF16 지원이면 BF16, 아니면 FP16
        self.amp_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16

        # FP16에만 GradScaler 사용 (BF16은 보통 불필요)
    ...
    def forward_train(self, data_batch, epoch, temperature=0.5):
        ...
        src_pred, map, features,others = self.student_forward(data_batch['src_left'], data_batch['src_right'])
        ...
        data_batch['occ_up'] = others[0]
        data_batch['occ_logit'] = others[1]
        ...
        # tgt_pred, map, features,others = self.student_forward(data_batch['tgt_left_strong_aug'], data_batch['tgt_right_strong_aug'])  
        ...
        # student_loss = 0.0#get_loss(data_batch['tgt_pred_disp_s_for_loss'], gt_tgt_disp, mask, weight)
        # teacher_loss = 0.0#get_loss(data_batch['pseudo_disp_for_loss'], gt_tgt_disp, mask, weight)
    ...
    def forward_test(self, data_batch, epoch):
        ...
        tgt_pred, map, features,others = self.student_forward(data_batch['tgt_left'], data_batch['tgt_right'])  
        data_batch['tgt_pred_disp_s'] = tgt_pred
        data_batch['tgt_disp_diff'] = map[0]
        data_batch['tgt_corr_volume_s_1'] = map[1]
        data_batch['tgt_mask_pred_s'] = map[2]
